...
class sqlite_interface:
	...
	def checkID(self,id):
		...
		for row in self._c.execute('''select * from errors where digitalID=?''',(id,)):
			dat.append(row)
 #       self._conn.close()
	...
	def AppendToDB(self,lst):
			...
			for row in lst:
				try:
					...
					errordata=(row['digitalID'],str(e),)
					self._c.execute('INSERT INTO errors(digitalID,error) VALUES(?,?)',errordata)
